...
class Block(nn.Module):
    def __init__(self, in_ch, out_ch, num_heads=1, sr_ratio=1, drop_path=0.):
        super().__init__()
        self.norm1 = nn.LayerNorm(in_ch, eps=1e-6)
        self.attn = New_GAU(in_ch, sr_ratio=sr_ratio)
        self.norm2 = nn.LayerNorm(in_ch, eps=1e-6)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        self.pe = PatchEmbed(in_ch, out_ch)

    def forward(self, x):
        B, C, H, W = x.size()
        x = x.flatten(2).transpose(1, 2)
        #x = x + self.drop_path(self.attn(self.norm1(x)))  # pre_norm
...
class PatchEmbed(nn.Module):
    def __init__(self, in_ch=3, out_ch=96, with_pos=True):
        super().__init__()
        self.proj = nn.Conv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1, groups=32)
...
class hybrid4(torch.nn.Module):
    ...
    def __init__(self, input_channels=3, num_classes=4, alpha=1.67):
        ...
        self.GA1 = Block(in_ch=self.filters[0], out_ch=self.filters[1])
        self.GA2 = Block(in_ch=self.filters[1], out_ch=self.filters[2])
        self.GA3 = Block(in_ch=self.filters[2], out_ch=self.filters[3])
        self.GA4 = Block(in_ch=self.filters[3], out_ch=self.filters[4])

        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.head = nn.Linear(self.filters[4], num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        hybrid4 = self.GA4(x_multires4)

        #hybrid5 = self.GA5(x_multires5)
        final = self.avg_pool(hybrid4).flatten(1)
