...
class BasicBlock(nn.Module):
	...
	def __init__(self, inplanes, planes, stride=1, downsample=None):
		...
		self.stride = stride
		self.conv = conv3x3(planes, planes, 1)

		self.norm = nn.LayerNorm(planes, eps=1e-6)
		self.attn = New_GAU(dim=planes)
		self.attn2 = Attention(dim=planes)
	...
	def forward(self, x):
		...
		B, C, H, W = out.size()
		#if out.size(1) == 512:
		#	out = self.norm(self.attn2(out.view(B, -1, C), H, W))
...
class ResNet(nn.Module):
	...
	def __init__(self, block, layers, num_labels=4):
		self.inplanes = 64
...
def resnet_hybrid(pretrained=False, **kwargs):
