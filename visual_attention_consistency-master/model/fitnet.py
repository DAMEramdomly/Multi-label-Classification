...
class ImageFeatureEmbedder(nn.Module):
    def __init__(self, *, channel, dim, image_size, patch_size, dropout=0.):
        super().__init__()
        # 注意：这里的 image_size 应为一个元组，表示图像的长和宽
    ...
    def __init__(
        ...
        *,
        ...
        self.sm_image_embedder = ImageFeatureEmbedder(
            channel=sm_channel,
            dim=sm_dim,
            image_size=(256, 512),  # 以元组形式传递图像的高度和宽度
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=4, fusionFlag=False):
